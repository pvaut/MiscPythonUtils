        ...
        for col in self.GetColList():
            ...
            if colinfo.GetTypeStr()=='Text':
                ...
                for rownr in self.GetRowNrRange():
                    maxlen = max(maxlen, len(str(self.GetValue(rownr, colnr))))
...
#            print(str(self.Columns[colnr]))
    ...
    def ConvertColToValue(self, ColName):
        ...
        coldata=col['data']
        for i in range(0,len(coldata)):
            if (coldata[i]!='None') and (coldata[i]!='NA') and (coldata[i]!='') and (coldata[i] is not None) and (coldata[i]!='inf') and (coldata[i]!='-'):
                try:
